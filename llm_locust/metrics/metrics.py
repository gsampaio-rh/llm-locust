...
from llm_locust.core.models import MetricsLog, RequestFailureLog, RequestSuccessLog
...
class QuantileMetric(SimpleMetric, ABC):
    ...
    def calculate(
        ...
        log_dict = {f"{self.name}_quantile_{q}": 0.0 for q in self.quantiles}
        if bucket:
            log_dict.update(
                {
                    f"{self.name}_quantile_{q}": float(np.percentile(bucket, q))
...
class OutputTokensMetric(SimpleMetric):
    ...
    def calculate(
        ...
        time_interval = end_timestamp - start_timestamp
        if time_interval > 0:
            return {"total_output_tokens_per_second": sum(bucket) / time_interval}
        return {"total_output_tokens_per_second": 0.0}
    ...
    def collect_request(
        ...
        if isinstance(request_log, RequestFailureLog) or metrics_collector is None:
            return
        total_tokens = 0
...
class EmptyTokensMetric(SimpleMetric):
    """Metrics based on empty output tokens"""
    ...
    def calculate(
        ...
        time_interval = end_timestamp - start_timestamp
        if time_interval > 0:
            return {"total_empty_output_tokens_per_second": sum(bucket) / time_interval}
        return {"total_empty_output_tokens_per_second": 0.0}
...
class TPOTMetric(QuantileMetric):
    """Time Per Output Token (TPOT) metric."""
...
class LLMMetricsList(MetricsList):
    """Complete metrics set for LLM load testing."""
